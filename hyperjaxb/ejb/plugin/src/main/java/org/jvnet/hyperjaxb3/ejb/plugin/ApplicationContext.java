...
public class ApplicationContext implements IApplicationContext {
        ...
        if (factory == null) {
            try {
                factory = JAXBContextFactory.createInstance(
                    "jakarta.xml.ns.persistence:jakarta.xml.ns.persistence.orm:org.jvnet.hyperjaxb3.ejb.schemas.customizations");
            } catch (Exception e) {
    ...
    public Naming getNaming() {
        if (naming == null) {
            naming = createNaming(getIgnoring());
        }
        return naming;
    }
    ...
    public Properties createReservedNames() {
        try {
            ...
            Iterator<URL> urls = this.getClass().getClassLoader().getResources("org/jvnet/hyperjaxb3/ejb/strategy/naming/impl/ReservedNames.properties").asIterator();
            while (urls.hasNext()) {
