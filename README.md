...
For the current version 3.X, its artifacts are named `org.jvnet.jaxb:jaxb-plugins`, while the previous version 2.X were named `org.jvnet.jaxb:jaxb2-basics`.

So, please replace all `jaxb2-basics` with `jaxb-plugins` in the following documentations (also in wiki) if you use version 3.X.
